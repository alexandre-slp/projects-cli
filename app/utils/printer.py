...
async def format_matrix(apps: dict) -> (list, int, int):
    ...
    async with asyncio.TaskGroup() as tg:
        for i, app_name in enumerate(apps_with_status):
            tg.create_task(get_app_name_width(app_name, app_names_width))
            tg.create_task(set_app_position(app_name, formatted_apps_in_position, i, num_rows))

    max_col_width = max(app_names_width)
    return formatted_apps_in_position, max_col_width, num_cols


async def get_app_name_width(app_name: str, app_names_width: [int]):
...
async def format_app_status(apps: dict) -> list:
    ...
    apps_with_status.sort(key=lambda x: re.fullmatch(word_pattern, x).group(1))
